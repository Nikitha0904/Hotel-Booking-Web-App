  ...
  const [isLoading, setIsLoading] = useState(false);
    ...
    function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        if (hotel && room) {
          ...
          axios.post('/api/room', {...values, hotelId: hotel.id})
            .then((res) => {
              toast({
                variant: "success",
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex flex-col space-y-3">
                                ...
                                <FormDescription>Choose an image that will showcase your room nicely</FormDescription>
                                <FormControl>
                                    {image ? <>
                    ...
                    <div className="pt-4 pb-2">
                        {room ? <Button onClick={form.handleSubmit(onSubmit)} type='button' className="max-w-[150px]" disabled={isLoading}>{isLoading ?
                          ...
                          h-4 w-4" />Update</>}</Button> : <Button onClick={form.handleSubmit(onSubmit)} type='button' className="max-w-[150px]" disabled=
                            {isLoading}>
                            {isLoading ? <><Loader2 className="mr-2 h-4 w-4" /> Creating</> : <><PencilLine
                                className="mr-2 h-4 w-4" />Create Room</>}
                        </Button>}
                    </div>
                </form>
            </Form>
