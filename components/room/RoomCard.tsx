...
import { AirVent, Bath, Bed, BedDouble, Castle, Home, Loader2, MountainSnow, Pencil, Ship, Trash, Trees, Tv, Users, UtensilsCrossed, VolumeX, Wifi } from "lucide-react";
        ...
        <Card>
            ...
            <CardContent className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4 content-start text-sm">
                    <AmenityItem><Bed className="h-4 w-4" />{room.bedCount} Bed{room.bedCount > 1 ? 's' : ''}</AmenityItem>
                    <AmenityItem><Users className="h-4 w-4" />{room.guestCount} Guest{room.guestCount > 1 ? 's' : ''}</AmenityItem>
                    <AmenityItem><Bath className="h-4 w-4" />{room.bathroomCount} Bathroom{room.bathroomCount > 1 ? 's' : ''}</AmenityItem>
                    {!!room.kingBed && <AmenityItem><BedDouble className="h-4 w-4" />{room.kingBed} King Bed{room.kingBed > 1 ? 's' : ''}</AmenityItem>}
                    {!!room.queenBed && <AmenityItem><Bed className="h-4 w-4" />{room.queenBed} Queen Bed{room.queenBed > 1 ? 's' : ''}</AmenityItem>}
                    ...
                    {room.airCondition && <AmenityItem><AirVent className="h-4 w-4" />Air Conditioned</AmenityItem>}
